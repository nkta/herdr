...
pub(crate) struct ClientShellConfig {
    ...
    pub(super) prefix_hint_delay: Option<std::time::Duration>,
...
pub(super) struct ShellHitMap {
    ...
    pub(super) git_commit_button: Rect,
    pub(super) git_file_list: Rect,
    pub(super) git_max_scroll: usize,
...
pub(crate) struct ClientShellState {
    ...
    pub(super) mode: ClientShellMode,
    /// Set once prefix mode has been held past the configured delay. Presentation
    /// state: the panel exists only in this client.
    pub(super) prefix_hint_visible: bool,
    pub(super) prefix_hint_deadline: Option<std::time::Instant>,
...
impl ClientShellState {
    ...
    /// panel correct however prefix mode was entered or left. Returns whether
    /// the panel's visibility changed, so the caller can recompose.
    pub(crate) fn tick_prefix_hint(&mut self, now: std::time::Instant) -> bool {
        let armed = self.mode == ClientShellMode::Prefix && self.config.prefix_hint_delay.is_some();
        if !armed {
            self.prefix_hint_deadline = None;
            return std::mem::take(&mut self.prefix_hint_visible);
        }
        ...
        let Some(delay) = self.config.prefix_hint_delay else {
            return false;
        };
    ...
    pub(crate) fn timer_delay(&self, now: std::time::Instant) -> std::time::Duration {
        ...
        self.selection_autoscroll_deadline
            ...
            .chain(self.prefix_hint_deadline)
            .min()
