...
/// Multi-row twin of [`restore_mode_bar`], for chrome that is taller than one line.
fn restore_panel(
    frame: &mut FrameData,
    panel: Option<Rect>,
    cells: Option<&[crate::protocol::CellData]>,
) {
    let (Some(panel), Some(cells)) = (panel, cells) else {
        return;
    };
    let width = usize::from(panel.width);
    for (row, chunk) in cells.chunks(width).enumerate() {
        let y = usize::from(panel.y) + row;
        let start = y * usize::from(frame.width) + usize::from(panel.x);
        frame.cells[start..start + width].clone_from_slice(chunk);
    }
    if frame
        .cursor
        .as_ref()
        .is_some_and(|cursor| cursor.y >= panel.y && cursor.y < panel.bottom())
    {
        frame.cursor = None;
    }
}

fn panel_cells(frame: &FrameData, panel: Rect) -> Vec<crate::protocol::CellData> {
    let width = usize::from(panel.width);
    (panel.y..panel.bottom())
        .flat_map(|y| {
            let start = usize::from(y) * usize::from(frame.width) + usize::from(panel.x);
            frame.cells[start..start + width].to_vec()
        })
        .collect()
}
...
impl ClientShellState {
    ...
    pub(crate) fn compose(&mut self, cols: u16, rows: u16) -> Option<FrameData> {
        ...
        // The panel sits above the mode bar, so it is painted into the same
        // buffer and restored after the pane surface is blitted over both.
        let prefix_hint = (self.prefix_hint_visible && self.overlay.is_none())
            .then(|| {
                render::render_prefix_hint_panel(
                    &mut buffer,
                    layout.pane_surface,
                    &self.config.keybinds,
                    &self.config.palette,
        ...
        let prefix_hint_cells = prefix_hint.map(|panel| panel_cells(&frame, panel));
        blit_pane_surface(&mut frame, &surface.frame, layout.pane_surface);
        restore_panel(&mut frame, prefix_hint, prefix_hint_cells.as_deref());
