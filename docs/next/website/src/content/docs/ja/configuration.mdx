...
Git パネルのコミットボックスでは、外部の CLI エージェントを使って、ステージ済みの差分からコミットメッセージを生成できます。エージェントはクライアントではなく、Herdr サーバーを実行しているマシン上で動作します。Git の作業ツリーとエージェントのバイナリがそこにあるためです。
...
`[[commit_agents.agents]]` はユーザー定義のリストのため、[設定リファレンス](/ja/docs/config-reference/)にはキーごとに掲載していません。エージェントごとに 1 つのエントリを追加し、一意の `id`、表示用の `label`、実行する `command`、その `args` を指定します。設定した `command` は標準入力からプロンプト（指示とステージ済みの差分）を読み取り、標準出力にコミットメッセージを出力する必要があります。`commit_agents.active` は実行するエージェントの id を選びます。設定画面の「commit agent」セクションから変更するか、直接編集してください。`[commit_agents]` セクションがなくても、Herdr には動作する `claude` エージェントが 1 つデフォルトで用意されています。

コミットボックスにフォーカスがある状態で `ctrl+g` を押すと、アクティブなエージェントでメッセージを生成し、入力済みの下書きを置き換えます。
